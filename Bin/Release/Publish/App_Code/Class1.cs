...
namespace Lock_ARM_Asp.Class
{
    public class Class1
    {
        ...
        int Nk_AES = 4, Nb_AES = 4, Nr_AES = 10;
        byte[,] State_AES = new byte[4, 4];
        byte[,] w_AES = new byte[4 * 11, 4];
        byte[] temp_AES = new byte[4];
        ...
        void SubWord()
        {
            temp_AES[0] = Sbox[temp_AES[0] >> 4, temp_AES[0] & 0x0f];
            temp_AES[1] = Sbox[temp_AES[1] >> 4, temp_AES[1] & 0x0f];
            temp_AES[2] = Sbox[temp_AES[2] >> 4, temp_AES[2] & 0x0f];
            temp_AES[3] = Sbox[temp_AES[3] >> 4, temp_AES[3] & 0x0f];
            return;
        }
        void SubBytes()
        {
            for (int r = 0; r < 4; ++r)
            {
                for (int c = 0; c < 4; ++c)
                {
                    State_AES[r, c] = Sbox[(State_AES[r, c] >> 4), (State_AES[r, c] & 0x0f)];
                }
            }
            return;
        }
        void AddRoundKey(int round)
        {
            ...
            for (int r = 0; r < 4; ++r)
            {
                for (int c = 0; c < 4; ++c)
                {
                    State_AES[r, c] = (byte)((State_AES[r, c]) ^ (w_AES[(round * 4) + c, r]));
                }
            }
            return;
        }
        ...
        {
            int row;
            for (row = 0; row < Nk_AES; ++row)
            {
                w_AES[row, 0] = AESKEY[4 * row];
                w_AES[row, 1] = AESKEY[4 * row + 1];
                w_AES[row, 2] = AESKEY[4 * row + 2];
                w_AES[row, 3] = AESKEY[4 * row + 3];
            }
            for (row = Nk_AES; row < Nb_AES * (Nr_AES + 1); ++row)
            {
                ...
                if (row % Nk_AES == 0)
                {
                    RotWord();
                    SubWord();
                    temp_AES[0] = (byte)(temp_AES[0] ^ Rcon1[row / Nk_AES]);
                }
                w_AES[row, 0] = (byte)(w_AES[row - Nk_AES, 0] ^ temp_AES[0]);
                w_AES[row, 1] = (byte)(w_AES[row - Nk_AES, 1] ^ temp_AES[1]);
                w_AES[row, 2] = (byte)(w_AES[row - Nk_AES, 2] ^ temp_AES[2]);
                w_AES[row, 3] = (byte)(w_AES[row - Nk_AES, 3] ^ temp_AES[3]);
            }  // for loop
            return;
        }
        void ShiftRows()
        {
            ...
            for (int r = 1; r < 4; ++r)  // shift temp1 into State
            {
                for (int c = 0; c < 4; ++c)
                {
                    State_AES[r, c] = temp1[r, (c + r) % 4];  //State_AES[r][c] = temp1[ r][ (c + r) % Nb_AES ];
                }
            }
            return;
        }
        void MixColumns()
        {
            ...
            for (int c = 0; c < 4; ++c)
            {
                State_AES[0, c] = (byte)(gfmultby02(temp1[0, c]) ^ gfmultby03(temp1[1, c]) ^ gfmultby01(temp1[2, c]) ^ gfmultby01(temp1[3, c]));
                State_AES[1, c] = (byte)(gfmultby01(temp1[0, c]) ^ gfmultby02(temp1[1, c]) ^ gfmultby03(temp1[2, c]) ^ gfmultby01(temp1[3, c]));
                State_AES[2, c] = (byte)(gfmultby01(temp1[0, c]) ^ gfmultby01(temp1[1, c]) ^ gfmultby02(temp1[2, c]) ^ gfmultby03(temp1[3, c]));
                State_AES[3, c] = (byte)(gfmultby03(temp1[0, c]) ^ gfmultby01(temp1[1, c]) ^ gfmultby01(temp1[2, c]) ^ gfmultby02(temp1[3, c]));
            }
            return;
        }
        void InvShiftRows()
        {
            ...
            for (int r = 1; r < 4; ++r)  // shift temp1 into State
            {
                for (int c = 0; c < 4; ++c)
                {
                    State_AES[r, (c + r) % Nb_AES] = temp1[r, c];
                }
            }
            return;
        }
        void InvSubBytes()
        {
            for (int r = 0; r < 4; ++r)
            {
                for (int c = 0; c < 4; ++c)
                {
                    State_AES[r, c] = iSbox[State_AES[r, c] >> 4, (State_AES[r, c] & 0x0f)];
                }
            }
            return;
        }  // InvSubBytes
        void InvMixColumns()
        {
            ...
            for (int c = 0; c < 4; ++c)
            {
                State_AES[0, c] = (byte)((int)gfmultby0e(temp1[0, c]) ^ (int)gfmultby0b(temp1[1, c]) ^ (int)gfmultby0d(temp1[2, c]) ^ (int)gfmultby09(temp1[3, c]));
                State_AES[1, c] = (byte)((int)gfmultby09(temp1[0, c]) ^ (int)gfmultby0e(temp1[1, c]) ^ (int)gfmultby0b(temp1[2, c]) ^ (int)gfmultby0d(temp1[3, c]));
                State_AES[2, c] = (byte)((int)gfmultby0d(temp1[0, c]) ^ (int)gfmultby09(temp1[1, c]) ^ (int)gfmultby0e(temp1[2, c]) ^ (int)gfmultby0b(temp1[3, c]));
                State_AES[3, c] = (byte)((int)gfmultby0b(temp1[0, c]) ^ (int)gfmultby0d(temp1[1, c]) ^ (int)gfmultby09(temp1[2, c]) ^ (int)gfmultby0e(temp1[3, c]));
            }
            return;
        }  // InvMixColumns
        public byte CheckAuthenticate(object obj, uint Param1)
        {
        ...
        public void ReverseAES(byte[] bteInput, ref byte[] output, byte[] Key)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < (4 * 4); i++)
                    {
                        State_AES[i % 4, i / 4] = bteInput[i];
                    }
                    AddRoundKey(Nr_AES);

                    for (int round = Nr_AES - 1; round >= 1; round--)  // main round loop
                    {
                        InvShiftRows();
                        InvSubBytes();
                        AddRoundKey(round);
        ...
        public void AES(byte[] input, byte[] output, byte[] Key)  // encipher 16-bit input
        {
            int i, round;
            try
            {
                ...
                for (i = 0; i < (4 * 4); i++)
                {
                    State_AES[i % 4, i / 4] = input[i];
                }
                AddRoundKey(0);
                for (round = 1; round <= (Nr_AES - 1); ++round)  // main round loop
                {
                    SubBytes();
                    ShiftRows();
                    MixColumns();
        ...
        public object CodeData(ARM.IWhichData iWhichData, string DataToWrite, byte[] Key)
        {
            byte[] bte;
            byte[] bteData;
            byte[] btestr;
            byte[] bte16 = new byte[16];
            byte[] bteData1 = new byte[16];
            byte[] intBytes;
            int Value, Len = 0;

            bte = System.Text.Encoding.Default.GetBytes(DataToWrite);
            if (iWhichData == ARM.IWhichData.DATA_PARTITION)
            {
                btestr = new byte[240];
                try
                {
                    bteData = new byte[240];
                    Len = bte.Length;
                    if (Len > 240)
                        Len = 240;
                    for (int i = 0; i < Len; i++)
            ...
            else if (iWhichData == ARM.IWhichData.DESCRIPTION || iWhichData == ARM.IWhichData.STRING_VAL2 || iWhichData == ARM.IWhichData.STRING_VAL1)
            {
                btestr = new byte[128];
                try
                {
                    bteData = new byte[128];
                    Len = bte.Length;
                    if (Len > 128)
                        Len = 128;
                    for (int i = 0; i < Len; i++)
            ...
            else if (iWhichData == ARM.IWhichData.PRODUCT_NAME)
            {
                btestr = new byte[16];
                try
                {
            ...
            else if (iWhichData == ARM.IWhichData.PRODUCT_VERSION)
            {
                btestr = new byte[16];
                try
                {
            ...
            else if (iWhichData == ARM.IWhichData.INT_VAL1 || iWhichData == ARM.IWhichData.INT_VAL2 || iWhichData == ARM.IWhichData.INT_VAL3 || iWhichData == ARM.IWhichData.INT_VAL4 || iWhichData == ARM.IWhichData.NT_USER)
            {
                btestr = new byte[16];
                try
                {
                    bteData = new byte[16];
                    Value = Convert.ToInt32(DataToWrite);
                    intBytes = BitConverter.GetBytes(Value);
                    Array.Reverse(intBytes);
                    for (int i = 0; i < 4; i++)
                        bteData[i] = intBytes[i];
                    for (int i = 4; i < 16; i++)
                        bteData[i] = 0;
                    AES(bteData, bteData1, Key);
                    for (int j = 0; j < 16; j++)
        ...
        public string DecodData(object obj, ARM.IWhichData iWhichData, uint RProg)
        {
            UInt16 DataCheckSum1 = 0, DataCheckSum2 = 0;
            ...
            string str = "";
            a1 = a2 = a3 = a4 = a5 = 0;
            if (iWhichData == ARM.IWhichData.SPECIAL_ID || iWhichData == ARM.IWhichData.SERIAL_NUM || iWhichData == ARM.IWhichData.PRODUCT_NAME)
            {
                btestr = (byte[])obj;
                try
                {
                    ...
                    {
                        if (iWhichData == ARM.IWhichData.SPECIAL_ID || iWhichData == ARM.IWhichData.PRODUCT_NAME)
                        {
                        ...
                        else if (iWhichData == ARM.IWhichData.SERIAL_NUM)
                        {
                            str = "";
            ...
            else if (iWhichData == ARM.IWhichData.DATA_PARTITION || iWhichData == ARM.IWhichData.DESCRIPTION || iWhichData == ARM.IWhichData.STRING_VAL1 || iWhichData == ARM.IWhichData.STRING_VAL2)
            {
                ...
                else if (iWhichData == ARM.IWhichData.DESCRIPTION || iWhichData == ARM.IWhichData.STRING_VAL1 || iWhichData == ARM.IWhichData.STRING_VAL2)
                {
                    a1 = 307;
                    a2 = 40;
                    a3 = 16;
            ...
            else if (iWhichData == ARM.IWhichData.PRODUCT_VERSION || iWhichData == ARM.IWhichData.NT_USER || iWhichData == ARM.IWhichData.INT_VAL1 || iWhichData == ARM.IWhichData.INT_VAL2 || iWhichData == ARM.IWhichData.INT_VAL3 || iWhichData == ARM.IWhichData.INT_VAL4)
            {
                if (iWhichData == ARM.IWhichData.PRODUCT_VERSION)
                {
                    a1 = 8;
                }
                else if (iWhichData == ARM.IWhichData.NT_USER || iWhichData == ARM.IWhichData.INT_VAL1 || iWhichData == ARM.IWhichData.INT_VAL2 || iWhichData == ARM.IWhichData.INT_VAL3 || iWhichData == ARM.IWhichData.INT_VAL4)
                {
                    a1 = 4;
                }
                btestr = (byte[])obj;
        ...
        public byte[] CreateUserKey(string str, byte[] Key)
        {
        ...
        public byte[] EncodeQueryRequest(string strRequest, byte[] Key)
        {
            byte[] bte1 = new byte[16];
            byte[] bte2 = new byte[16];
            try
            {
                bte2 = HexStringToByteArray(strRequest);
                AES(bte2, bte1, Key);
            }
            catch { }
            return bte1;
        ...
        public string DecodeQueryResponse(object OBJ, byte[] Key)
        {
            string str = "";
            byte[] bte1 = new byte[16];
            byte[] bte2 = new byte[16];
            ...
            try
            {
                ReverseAES(bte1, ref bte2, Key);
                str = BitConverter.ToString(bte2).Replace("-", string.Empty);
            }
            catch { }
            return str;
        }

        public byte[] EnCodeDataEquation(byte Param1, byte Param2, byte Param3, byte Param4, byte[] Key)
        {
            byte[] bte1 = new byte[16];
            ...
            try
            {
                ...
                for (int i = 4; i < 15; i++)
                {
                    bteRand[i] = (byte)(GetRand.Next(255));
                }
        ...
        public byte[] EncodeQueryRequestAES(string strRequest, byte[] Key)
        {
            byte[] bte1 = new byte[16];
            byte[] bte2 = new byte[16];
            try
            {
                bte2 = HexStringToByteArray(strRequest);
                AES(bte2, bte1, key_AES);
            }
            catch { }
            return bte1;
        }

        public string DecodeQueryResponseAES(object OBJ, byte[] Key)
        {
            string str = "";
            byte[] bte1 = new byte[16];
            byte[] bte2 = new byte[16];
            ...
            try
            {
                ReverseAES(bte1, ref bte2, key_AES);
                str = BitConverter.ToString(bte2).Replace("-", string.Empty);
            }
            catch { }
            return str;
        ...
        public uint FuncSecond(object obj)
        {
            double p;
            uint y = 0;
            byte[] btedata = new byte[70];
            ...
            try
            {
                p = BitConverter.ToDouble(btedata, 43);
                y = Func64(p, 55);
            }
            catch { }
            return y;
        }
        public uint FuncBig(double p, double k, double z)
        {
